...
def test_open_ai_selected_config():
    settings = load_settings("tests/data/test_settings_open_ai.toml", reload_settings=True)
    assert settings.selected_config == OpenAiConfig(api_key="Your OpenAI API key", model="default")

def test_gemini_selected_config():
    settings = load_settings("tests/data/test_settings_gemini.toml", reload_settings=True)
    assert settings.selected_config == GeminiAiConfig(api_key="Your Gemini API key", model="default")

def test_mistral_selected_config():
    settings = load_settings("tests/data/test_settings_mistral.toml", reload_settings=True)
...
def test_load_settings_with_debug_mode_enabled():
    settings = load_settings("tests/data/test_settings_debug_enabled.toml", reload_settings=True)
    assert settings.debug_mode

def test_load_settings_with_debug_mode_enabled():
    settings = load_settings("tests/data/test_settings_default.toml", reload_settings=True)
